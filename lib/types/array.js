...
Schematik.Array = function (options) {
  ...
  this.setType('array');
  if (options && options.flags.unique)
...
Schematik.Array.prototype.done = function () {
  ...
  if (this.items) {
    if (!_.isArray(this.items))
      schema.items = Schematik.util.resolve(this.items, {forceOptional: true});
    // If this.items is an array
    else
      schema.items = Schematik.util.resolveAll(this.items, {forceOptional: true});
  }
  // If additionalItems is a boolean
  if (this.additionalItems) {
    schema.additionalItems = Schematik.util.resolve(this.additionalItems, {forceOptional: true});
  }
