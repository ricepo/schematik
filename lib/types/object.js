...
Schematik.Object = function (options) {
  this._props      = { };
  this._pattern    = { };
  this._additional = Schematik.config.allowAdditionalProperties ? true : null;
...
Schematik.Object.prototype.done = function () {
  ...
  if (Object.keys(this._props).length !== 0) {
    schema.properties = { };
    _.map(this._props, function (v, k) {
      schema.properties[k] = Schematik.util.resolve(v);
    });
  }

  // Resolve pattern properties
  if (Object.keys(this._pattern).length !== 0) {
    schema.patternProperties = { };
    _.map(this._pattern, function (v, k) {
      schema.patternProperties[k] = Schematik.util.resolve(v);
    });
  }

  // Resolve additional property
  if (_.isBoolean(this._additional) || _.isObject(this._additional)) {
    schema.additionalProperties = Schematik.util.resolve(this._additional, { forceOptional: true });
  }
...
Chainable.func(Schematik.Object, 'property', function (key, schema) {
  ...
  if (self.flags.pattern) {
    ...
    if (self._pattern[key])
      throw new Error('Schematik.Object.property: cannot overwrite pattern property ' + key);
  ...
  // If this is an additional property
    ...
    // Do not allow overwrites
    if (self._additional !== Schematik.config.allowAdditionalProperties && !_.isNull(self._additional))
      throw new Error('Schematik.Object.property: cannot overwrite additional property.');

    // If no parameters specified, check for {no} flag
    if (arguments.length === 0)
      this._additional = !this.flags.no;
    else {
      // Check argument type
      if (!_.isBoolean(arguments[0]) && !_.isObject(arguments[0]))
        throw new Error('Schematik.Object.property: additional property must be a boolean or object.');
      this._additional = arguments[0];
    }
  }
  ...
  // If this is a normal property
    ...
    // Do not allow overwrites
    if (self._props[key])
      throw new Error('Schematik.Object.property: cannot overwrite property ' + key);
