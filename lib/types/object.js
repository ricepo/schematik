...
Chainable.func(Schematik.Object, 'property', function(key, schema) {
  ...
  // If this is a normal property
    ...
    // If the property has the required flag, add to the required list
    if (schema.attrib && schema.attrib.required) {
      // Create the `required` list if one is not already present
      if (!self.schema.required) { self.schema.required = []; }

      // Insert the property name
      self.schema.required.push(key);
    }
  }
